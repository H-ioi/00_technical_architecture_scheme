 ...
 *
 * 未传 `locale` 时会读 `storage.get('locale')`（须已设置正确的 `config.name` / `createUniLibI18n({ name })`）。
 */
export function createUniLibI18n(options: CreateUniLibI18nOptions = {}) {
  ...
  const locale =
    options.locale ??
    (fromStorage && isUniAppLocale(fromStorage) ? fromStorage : undefined) ??
    UNI_DEFAULT_LOCALE;
