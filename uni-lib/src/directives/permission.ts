...
const isEmptyPermission = (code: unknown) => !code || (Array.isArray(code) && code.length === 0)
...
const applyAllowedMode = (el: HTMLElement) => {
  ...
  if (originalState.placeholder?.parentNode) {
    originalState.placeholder.parentNode.insertBefore(el, originalState.placeholder)
...
export const uniPermissionDirective: Directive<HTMLElement, UniPermissionValue> = {
  mounted(el, binding) {
...
export const setupPermissionDirective = (app: App, nextOptions?: UniPermissionOptions) => {
