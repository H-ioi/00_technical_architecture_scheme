<template>
  <div class="uni-layout-tags">
    ...
    <div class="uni-layout-tags__actions">
      <el-button :icon="ArrowLeft" text :aria-label="translate('common.scrollLeft', '向左滚动')" @click="scrollTags('left')" />
      <el-button :icon="ArrowRight" text :aria-label="translate('common.scrollRight', '向右滚动')" @click="scrollTags('right')" />
      <el-dropdown
        trigger="click"
...
const syncVisitedTagsFromRoute = () => {
  ...
  const detailId = paramKey ? getRouteParamText(route.params[paramKey] as string | string[] | undefined) : ''
  const titleKey = detailId ? undefined : (meta.titleKey as string | undefined)
  const title = detailId ? `${String(meta.title || String(leaf.name))}_${detailId}` : String(meta.title || String(leaf.name))
