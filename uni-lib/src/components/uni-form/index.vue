  ...
  <el-form ref="formRef" class="uni-form" :model="formModel" :rules="config.rules" v-bind="config.formProps">
    <template v-for="section in groupedSections" :key="section.title || 'default'">
      ...
      <el-row v-bind="mergedRowProps">
        <el-col v-for="field in section.fields" :key="field.field" v-bind="field.colProps ?? section.colProps ?? config.colProps">
          <el-form-item :label="field.label" :prop="field.field" v-bind="field.formItemProps">
            <slot v-if="$slots[`field-${field.field}`]" :name="`field-${field.field}`" :field="field" :model="formModel" />
            ...
            <el-select
              ...
              v-bind="field.componentProps"
              @update:model-value="(value: unknown) => handleFieldChange(field, value)">
              <el-option v-for="option in getFieldOptions(field)" :key="String(option.value)" v-bind="option" :title="option.label" />
            </el-select>
...
const dynamicState = reactive<Record<string, { visible?: boolean; disabled?: boolean; options?: UniOption[] }>>({})
...
function normalizeRowProps(rowProps?: Partial<RowProps>): Partial<RowProps> {
  ...
  const n = typeof gutter === 'number' ? gutter : typeof gutter === 'string' ? Number.parseFloat(gutter) : Number(gutter)
...
const isFieldReadonly = (field: UniFormField) => Boolean(props.config.readonly) || resolveBoolean(field.readonly, field, false)
const getFieldOptions = (field: UniFormField) => dynamicState[field.field]?.options ?? field.options ?? []
...
const groupedSections = computed(() => {
  ...
  return props.config.sections.map((section) => ({
    ...section,
    fields: section.fields.map((field) => fieldMap.value.get(field)).filter(Boolean) as UniFormField[]
  }))
})
...
const renderViewValue = (field: UniFormField) => {
  ...
  if (field.viewType === 'date' || field.viewType === 'datetime') {
    return formatDate(context.value, field.viewType === 'date' ? 'YYYY-MM-DD' : 'YYYY-MM-DD HH:mm:ss')
  }
