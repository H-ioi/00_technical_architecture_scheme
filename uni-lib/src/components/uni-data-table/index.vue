<template>
  <div ref="rootRef" class="uni-data-table" :class="{
    ...
    <el-table v-loading="actualLoading" :data="actualData" :row-key="rowKey" :empty-text="actualEmptyText"
      :border="tableBorder" :height="actualHeight" :size="tableSize" :stripe="tableStripe" v-bind="elTableTreeBindings"
...
const props = withDefaults(
  defineProps<{
    ...
    emptyText?: string
    height?: number | string | false | 'auto'
    heightOffset?: number
...
const updateAutoMaxHeight = () => {
  if (props.height !== 'auto' || typeof window === 'undefined') {
    return
  }
  ...
  const availableHeight =
    window.innerHeight - tableTop - paginationHeight - paginationGap - props.heightOffset - 25
