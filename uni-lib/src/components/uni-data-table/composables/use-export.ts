...
export function useExport(options: {
  ...
  const { t } = useUniI18n()
  const getCellText = (row: Recordable, column: UniTableColumn, index: number) =>
    formatTableCellText(row, column, row[column.prop], index, options.getValueEnums?.(), t)
  ...
  const exportCurrentData = () => {
    ...
    const csv = [
      ...
      ...rows.map((row, index) => columns.map((column) => escapeCsvCell(getCellText(row, column, index))).join(','))
    ].join('\n')
