<template>
  <el-upload
    ...
    <slot name="trigger">
      ...
      <el-button v-else type="primary">{{ t('upload.trigger') }}</el-button>
    </slot>
...
const beforeUpload: UploadProps['beforeUpload'] = (file) => {
  if (!isAcceptedFile(file)) {
    emit('validate-error', t('upload.accept', { accept: props.accept }), file)
    return false
  }

  if (props.maxSize && file.size > props.maxSize) {
    emit('validate-error', t('upload.maxSize', { size: props.maxSize }), file)
    return false
  }

  if (props.maxTotalSize && fileListSize() + file.size > props.maxTotalSize) {
    emit('validate-error', t('upload.maxTotalSize', { size: props.maxTotalSize }), file)
    return false
  }
...
const handleSuccess: UploadProps['onSuccess'] = (response, file, files) => emit('success', response, file, files)
const handleError: UploadProps['onError'] = (error, file, files) => emit('error', error, file, files)
const handleRemove: UploadProps['onRemove'] = (file, files) => emit('remove', file, files)
const handlePreview: UploadProps['onPreview'] = (file) => emit('preview', file)
const handleExceed: UploadProps['onExceed'] = (files, uploadFiles) => emit('exceed', files, uploadFiles)
const handleProgress: UploadProps['onProgress'] = (event, file, files) => emit('progress', event as ProgressEvent, file, files)
