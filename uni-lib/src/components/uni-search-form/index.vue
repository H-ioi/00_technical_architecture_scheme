  ...
  <div class="uni-search-form">
    <div class="uni-search-form__body">
      <div class="uni-search-form__fields">
        <UniForm v-model="formModel" :config="searchFormConfig" @field-change="(payload) => emit('field-change', payload)" />
      </div>
      ...
      <div class="uni-search-form__actions-wrap">
        <slot
          ...
          <div class="uni-search-form__actions">
            ...
            <el-button v-if="needCollapse" class="uni-search-form__collapse" link type="primary" size="small" @click="toggleCollapsed">
    ...
    <div v-if="showSelectedTags && selectedTags.length" class="uni-search-form__selected-tags">
      <slot name="selected-tags" :tags="selectedTags" :remove="removeSelectedTag">
        <el-tag v-for="tag in selectedTags" :key="tag.field" closable @close="removeSelectedTag(tag.field)">{{ tag.label }}：{{ tag.value }}</el-tag>
      </slot>
    </div>
  </div>
</template>
...
const getFieldSpan = (field: UniFormField) => Number(field.colProps?.span ?? props.config.colProps?.span ?? GRID_COLUMNS)

const totalFieldSpan = computed(() => props.config.schema.reduce((total, field) => total + getFieldSpan(field), 0))

const maxCollapsedFieldSpan = computed(() => Math.max(GRID_COLUMNS, props.collapsedRows * GRID_COLUMNS) - props.actionMinSpan)
