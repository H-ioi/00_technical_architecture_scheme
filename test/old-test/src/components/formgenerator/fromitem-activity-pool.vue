    ...
    >
      <div style="width: 100%" v-for="(item, index) in formArr" :key="index">
        <el-form-item
          ...
          :label="
            item.type == 'banner' || item.type == 'guardian' ? '' : item.label
          "
          :prop="item.id"
          :ref="item.id"
          :style="`width:100%`"
        ...
        >
          ...
          <!-- 嵌套表单 -->
          <div class="guardian" v-if="item.type == 'guardian'">
            <div
              v-for="(childItem, index) in childForm[item.fieldId]"
              :key="childItem"
            >
              <div class="guardian-btn">
                <el-button
                  v-if="childForm[item.fieldId].length > 1"
                  @click="delGuardian(item, childItem, index)"
                  round
                  type="text"
                  size="small"
                >
              ...
              <FromitemChild
                :ref="`guardianChild${childItem}`"
                :template="item"
              />
            </div>
            <div class="guardian-btn">
              <el-button
                @click="addGuardian(item)"
                round
                type="primary"
                size="small"
                icon="el-icon-plus"
...
export default {
  ...
  methods: {
    ...
    async reSetDynamic(data) {
      const { templateFields } = data;
      const dynamicFields = templateFields.filter(
        (item) => item.type !== "association"
      );

      for (const item of dynamicFields) {
        const fromItem = this.formArr.find(
          (k) => k.id === item.templateFormFieldId
        );
        if (!fromItem) continue;

        const { type, properties } = fromItem;
        const { option_multi, option } = properties;
        ...
        if (["select", "checkbox"].includes(type)) {
          ...
          const value = item.value
            ...
            .split(",");
          const idArr = option.reduce((acc, o) => {
          ...
          this.formArrValue[item.templateFormFieldId] =
            isCheckbox || option_multi ? idArr : String(idArr);
        } else if (type === "datetimepicker") {
          const date = JSON.parse(item.value);
          const datetime_type = properties.datetime_type;
          const isArr = dateTimeType.some(
            (d) => d.type === datetime_type && d.isArr
          );
      ...
      return new Promise((resolve, reject) => {
        this.$refs["form"].validate(async (valid) => {
          if (valid) {
            if (this.checkData()) {
              const fields = [];
              this.formArr.forEach(async (item) => {
                const obj = {
                ...
                if (item.type == "guardian") {
                  ...
                  if (item.type == "guardian") {
                    let childfields = [];
                    let childForm = this.childForm[item.fieldId] || [];
                    if (childForm.length > 0) {
                      childForm.forEach(async (child) => {
                        const childTemplates = await this.$refs[
                          `guardianChild${child}`
                        ][0].saveFormArrValue();
                        // 判断是否通过校验，不通过则返回空数组
                        if (!childTemplates.status) {
                          return;
                        }
                        childfields.push({
                          fields: childTemplates.childTemplates || [],
                        });
                      });
                    }
                    obj.childTemplates = childfields;
                    fields.push(obj);
                  }
                }
              });
              ...
              Object.keys(formDataClone).forEach(async (fieldId) => {
                ...
                if (formItem.type == "autoFill") {
                  ...
                  this.autoFillinfo = {
                    ...this.autoFillinfo,
                    collectionId: this.templateFormId,
                  };
                  // const res = await autoFill(this.autoFillinfo);
                }
    ...
    async reSetForm(template, collectionId) {
      ...
      const data = template.templateFields;
      this.formArr = [];
      ...
      for (const item of filteredAndSortedData) {
        ...
        if (item.type == "guardian") {
          // 设置数组
          this.childForm[item.fieldId] = [createCode()];
        }
    ...
    // 监护人
    addGuardian(item) {
      this.childForm[item.fieldId].push(createCode());
      console.log("addGuardian", item, this.childForm);
