  ...
  <div class="thepool_page">
    <div class="pool-search">
        ...
        <div>
          ...
          <div v-if="showMore">
            ...
            <div class="form-item-center">
              <label class="form-label">{{ $t("consult.创建人") }}：</label>
              <CreatorTree
                style="width: 240px"
                ref="CreatorTree"
                size="small"
            ...
            <div class="form-item-center">
              <label class="form-label">{{ $t("consult.跟进人") }}：</label>
              <FollowerTree
                style="width: 240px"
                ref="FollowerTree"
                size="small"
      ...
      >
        ...
        >
          ...
          <el-form-item :label="$t('consult.创建人')" style="width: 214px">
            <CreatorTree
              ref="CreatorTree"
              size="small"
              :treeData="treeData"
          ...
          <el-form-item :label="$t('consult.跟进人')" style="width: 214px">
            <FollowerTree
              ref="FollowerTree"
              size="small"
              :treeData="treeData"
    ...
    <div class="pool-tableBox">
      <div class="df_sb palyTableBox" style="padding-top: 0">
        <div class="df_sb">
          <el-button
            v-if="permissions['enquiry_all_add']"
            type="primary"
            size="small"
            round
            @click="showAdd = true"
          ...
          >
          <el-button
            v-if="permissions['enquiry_all_follower_add']"
            type="primary"
            size="small"
            round
          ...
          >
          <el-button
            v-if="tableData.length > 0 && permissions['enquiry_all_export']"
            type="defult"
            size="small"
            round
            @click="exportList"
          ...
          <el-button
            type="defult"
            v-if="permissions['enquiry_all_batch_edit']"
            size="small"
            round
          ...
          <el-button
            ...
            v-if="
              ((currentstatus == '1' || currentstatus == '0') &&
                permissions['enquiry_all_status_batch_close']) ||
              (currentstatus == '2' &&
          ...
          <el-button
            type="defult"
            v-if="
              permissions['enquiry_all_status_batch_active'] &&
              currentstatus == '4'
            "
            size="small"
            round
          ...
          >
          <el-button
            v-if="tableData.length > 0 && permissions['clue_send_email']"
            type="defult"
            size="small"
            round
            @click="sendEmail"
    ...
    <SelectSchool
      ...
      @exportHybridList="exportHybridList"
    />
...
import FollowerTree from "@/components/thepoolcommon/UserTree";
import CreatorTree from "@/components/thepoolcommon/UserTree";
...
export default {
  ...
  data() {
    return {
      ...
      // statusList: [],
      statusList: consult["statusAllList"],
  ...
  async created() {
    this.treeData = await getDptUserList();
  ...
  methods: {
    ...
    resetTableTitle() {
      ...
      if (thePool_LocalCache["enquiryTableList"]) {
        this.tableTitle = thePool_LocalCache["enquiryTableList"].filter(
          (item) => {
            return item["show"];
          }
        );
    ...
    getstatusList() {
      this.statusList = [];
      consult["statusAllList"].map((item) => {
        let obj = {
          ...item,
    ...
    changeKeywords(e) {
      console.log("changeKeywords", e);
      if (this.searchFrom["keyword"] == "") {
        this.searchFrom["schools"] = [];
    ...
    searchKeywords() {
      ...
      searchKeywords({ keyword: this.searchFrom["keyword"] }).then((res) => {
        if (res.data.success) {
          this.schoolList = [];
          this.enrollLevelsList = [];
          this.searchFrom["schools"] = [];
          this.searchFrom["enrollLevels"] = [];
          this.searchFrom["directions"] = [];
          this.searchFrom["channels"] = [];
          let { schools, enrollLevels } = res.data.data;
          let schoolValue = [];
          schools.map((item) => {
            Object.keys(item).forEach((obj) => {
              schoolValue.push(obj);
            });
          });
          this.schoolList = this.pooldictpermissions.filter((item) => {
            return schoolValue.includes(item.value);
          });
        }
      });
    },
    getClueList() {
      // 如果搜索关键词为空且无分页接口权限则不发起请求
      if (
        (!this.searchData["keyword"] || this.searchData["keyword"] == "") &&
        !this.permissions["enquiry_all_paginate"]
      )
        return;
      getClueList({
        ...this.pagination,
    ...
    exportHybridList() {
      ...
      delete data["pageSize"];
      delete data["pageNum"];
      exportHybridList(data).then((res) => {
        console.log("res", res);
        this.$message.success(this.$t("consult.成功"));
        download(res.data, res.headers["content-disposition"]);
      });
    ...
    rowClick(row) {
      //   this.$router.push("/thepool/enquiry/detail?clueId=" + row.id + "&type=1");
    ...
    getSearchData() {
      ...
      searchData["creatorIds"] = this.searchFrom["creatorIds"];
      searchData["followerIds"] = this.searchFrom["followerIds"];
      if (
    ...
    clear() {
      ...
      if (this.$refs["CreatorTree"]) {
        this.$refs["CreatorTree"].clearselect();
      }
      if (this.$refs["FollowerTree"]) {
        this.$refs["FollowerTree"].clearselect();
      }
    ...
    async selectAll() {
      ...
      let clueIds = await getClueIds(data);
    ...
    async addRequestParam(name) {
      ...
      this.$message.success(this.$t("consult.成功"));
      this.requestList = await getRequestParamList({ type: "1" });
    },
    async initRequestParam() {
      this.requestList = await getRequestParamList({ type: "1" });
      // 如果路径有keyword参数，就使用keyword参数作为查询条件
    ...
    setSearchData(data) {
      ...
      this.$nextTick(() => {
        ...
        if (this.searchFrom.creatorIds.length > 0) {
          if (this.$refs["CreatorTree"]) {
            let userName = this.userList.map((item) => {
              if (this.searchFrom.creatorIds.includes(String(item.value))) {
                return item.label;
              }
            });
            this.$refs["CreatorTree"].labelModel = userName;
            this.$refs["CreatorTree"].checkedId = this.searchFrom.creatorIds;
          }
        }
        if (this.searchFrom.followerIds.length > 0) {
          if (this.$refs["FollowerTree"]) {
            let userName = this.userList.map((item) => {
              if (this.searchFrom.followerIds.includes(String(item.value))) {
                return item.label;
              }
            });
            this.$refs["FollowerTree"].labelModel = userName;
            this.$refs["FollowerTree"].checkedId = this.searchFrom.followerIds;
          }
        }
    ...
    changeFollowStatus(val) {
      console.log("changeFollowStatus", this.searchFrom, val);
    },
  },
};
</script>
