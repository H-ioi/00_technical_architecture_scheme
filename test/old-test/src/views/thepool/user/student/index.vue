  ...
  <div class="thepool_page">
    ...
    <div class="pool-tableBox">
      <div class="df_sb palyTableBox" style="padding-top: 0">
        <div class="df_sb">
          <el-button
            v-if="permissions['thepool_user_student_add']"
            type="primary"
            size="small"
            round
            @click="addStudent"
          ...
          >
          <el-button
            v-if="permissions['thepool_user_student_qrcode']"
            type="primary"
            size="small"
            round
          ...
          <el-button
            v-if="
              tableData.length > 0 &&
              currentstatus == '1' &&
              permissions['thepool_user_student_enternotice']
            "
            type="primary"
            size="small"
            round
          ...
          >
          <el-button
            v-if="permissions['thepool_user_student_batchedit']"
            type="defult"
            size="small"
            round
          ...
          >
          <el-button
            v-if="permissions['thepool_user_student_import']"
            type="defult"
            size="small"
            round
          ...
          <el-button
            v-if="
              tableData.length > 0 && permissions['thepool_user_student_export']
            "
            type="defult"
            size="small"
            round
    ...
    <changeStatus
      ...
      :studentIds="studentIds"
      :batchStuentList="batchStuentList"
      :isMultiple="true"
    ...
    <UpgradeTable ref="UpgradeTable" :isMine="false" />
    <SaveRequestParam
...
export default {
  ...
  mounted() {
    ...
    this.$store.subscribeAction((action) => {
      ...
      if (this.$route.path != "/thepool/user/student/index") {
        return;
      }
  ...
  methods: {
    ...
    getList() {
      ...
      if (
        ...
        !this.permissions["enquiry_student_paginate"]
      )
        return;
      getStudentList({
        ...this.pagination,
    ...
    exportList() {
      ...
      delete data["pageNum"];
      exportStudentList(data).then((res) => {
        console.log("res", res);
        this.$message.success(this.$t("consult.成功"));
    ...
    playTab(name, item, scope) {
      switch (name) {
        case "look":
          this.$router.push("/student/detail?id=" + item.id + "&type=2");
          break;
        case "edit":
    ...
    rowClick(row, column, event) {
      ...
      this.$router.push("/student/detail?id=" + row.id + "&type=2");
    },
    // 分页
    ...
    changeStasus(item, index) {
      ...
      console.log("tableBtn", this.tableBtn);
      this.pagination["pageNum"] = 1;
      if (item.type == "-1") {
        this.searchFrom = {
          ...this.searchFrom,
    ...
    async addRequestParam(name) {
      ...
      this.$message.success(this.$t("consult.成功"));
      this.requestList = await getRequestParamList({ type: "5" });
    },
