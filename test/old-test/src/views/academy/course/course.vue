  ...
  <div>
    <div class="searchFromBox">
      <div class="df_sb searchFromBox_header">
        <div class="searchFromBox_header_titel">课程管理</div>
      </div>
    ...
    <div class="df_sb palyTableBox">
      <div>
        <el-button
          v-if="permissions['isaic_course_add']"
          type="primary"
          size="medium"
          @click="$router.push('/academy/course/editcourse?type=add')"
          >添加课程</el-button
        >
        ...
        >
          <template slot-scope="scope">
            <div class="df_align_center table_textbtn">
              ...
              <span v-if="permissions['isaic_course_edit']">
                <el-button
              ...
              <span
                v-if="
                  permissions['isaic_course_release'] &&
              ...
              <span
                v-if="
                  permissions['isaic_course_release'] && scope.row.release === 1
                "
              >
                <el-button
                  type="text"
              ...
              <span v-if="permissions['isaic_course_download_signup']">
                <el-button
                  type="text"
                  size="small"
              ...
              <span v-if="permissions['isaic_course_del']">
                <el-button
                  :disabled="scope.row.release == 1"
                  type="text"
                  size="small"
...
import CourseForm from "./formlist/courseform.vue";
...
import {
  ...
  upCourse,
  delCourse,
  downloadCourseSignup
} from "@/api/academy/course.js";
...
export default {
  ...
  methods: {
    getTabelData() {
      ...
      getCoursePage(data).then(res => {
        console.log("getCoursePage", res);
        if (res.data.success) {
    ...
    playTab(type, row, scope) {
      switch (type) {
        case "look":
          this.$router.push(`/academy/course/coursedetail?type=1&id=${row.id}`);
          break;
        case "edit":
          this.$router.push(
            "/academy/course/editcourse?type=edit&id=" + row.id
          );
          break;
        case "up":
        ...
        case "down":
          ...
          break;
        case "download":
          downloadCourseSignup(row["id"]).then(res => {
            this.$message({
              type: "success",
              message: "下载成功"
            });
