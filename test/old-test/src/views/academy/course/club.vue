  ...
  <div>
    <div class="searchFromBox">
      <div class="df_sb searchFromBox_header">
        <div class="searchFromBox_header_titel">俱乐部管理</div>
      </div>
      <ClubForm @search="search" />
    </div>
    <div class="df_sb palyTableBox">
      <div>
        <el-button
          v-if="permissions['isaic_club_add']"
          type="primary"
          size="medium"
          @click="$router.push('/academy/course/editclub?type=add')"
          >添加俱乐部</el-button
        >
        ...
        >
          <template slot-scope="scope">
            <div class="df_align_center table_textbtn">
              ...
              <span v-if="permissions['isaic_club_edit']">
                <el-button
              ...
              <span
                v-if="
                  permissions['isaic_club_release'] &&
              ...
              <span
                v-if="
                  permissions['isaic_club_release'] && scope.row.release === 1
                "
              >
                <el-button
                  type="text"
              ...
              <span v-if="permissions['isaic_club_download_signup']">
                <el-button
                  type="text"
                  size="small"
              ...
              <span v-if="permissions['isaic_club_del']">
                <el-button
                  :disabled="scope.row.release == 1"
                  type="text"
                  size="small"
...
import ClubForm from "./formlist/clubform.vue";
...
export default {
  ...
  data() {
    return {
      ...
      paginationTotal: 0,
      tableTitle: course["clubTableTitle"],
  ...
  methods: {
    getTabelData() {
      ...
      getClubPage(data).then(res => {
        console.log("getClubPage", res);
        if (res.data.success) {
    ...
    playTab(type, row, scope) {
      switch (type) {
        case "look":
          this.$router.push(`/academy/course/coursedetail?type=4&id=${row.id}`);
          break;
        case "edit":
          this.$router.push("/academy/course/editclub?type=edit&id=" + row.id);
          break;
        case "up":
        ...
        case "down":
          ...
          break;
        case "download":
          downloadClubSignup(row["id"]).then(res => {
            this.$message({
              type: "success",
              message: "下载成功"
            });
