   ...
   <div class="community_page">
    ...
    <div class="community_centent">
     ...
     <el-form :inline="true" :model="dataForm" @keyup.enter.native="getDataList()" style="display: flex; align-items: center;">
      ...
      <el-table-column prop="school" header-align="center" align="center" label="学校" show-overflow-tooltip>
...
import { getSchoolList } from '@/api/isacommunity/holiday'
...
export default {
  ...
  methods: {
    ...
    async getDataList() {
      ...
      try {
        const res = await getConfigList2({
          school: this.dataForm.school
        });
        this.dataList = res || [];
      } catch (error) {
        console.error('获取数据列表失败:', error); // Log the error for debugging
        this.dataList = [];
        this.$message.error('获取数据列表失败'); // User-friendly error message
      } finally {
    ...
    async deleteItem(id) {
      ...
      try {
        await this.$confirm(`确定要删除选中的${ids.length}条数据吗？`, '提示', {
        ...
        if (error !== 'cancel') { // User clicked cancel, do nothing
