<template>
  <div class="community_page">
    <div class="community_top">
      <div class="community_top_title">{{ $t("isagroup.公告内容") }}</div>
      <div class="community_top_btn">
        <el-button
          v-if="permissions['announcement_add']"
          type="primary"
          size="large"
          @click="showForm('add')"
          >{{ $t("btn.新增") }}</el-button
    ...
    <div class="community_centent">
      ...
      <div class="isa_table">
        <Table
          ref="Table"
          :showSelection="false"
          :tableTitle="tabletitle['contentAnnouncementTable']"
          :tableData="tableData"
          :tableBtn="tableBtn"
          :height="520"
          @playTab="playTab"
...
import {
  ...
  getContentAnnouncementPage,
  delContentAnnouncement,
  ...
  methods: {
    ...
    getList() {
      getContentAnnouncementPage({
        ...
        if (res.data.success) {
          console.log("getContentAnnouncementPage", res.data.data);
    ...
    formatData() {
      this.tableData.map((item) => {
        item["urgencyLevelLabel"] = this.$getListLabel(
          consts["urgencyLevel"],
          item["urgencyLevel"]
        );
    ...
    playTab(name, item, scope) {
      ...
      switch (name) {
        ...
        case "del":
          ...
          delContentAnnouncement(formData).then((res) => {
            if (res.data.success) {
    ...
    delData() {
      ...
      if (selectionId.length == 0) {
        ...
        this.$alert(
          ...
          delContentAnnouncement({ ids: selectionId }).then((res) => {
            if (res.data.success) {
              this.$message.success(this.$t("isagroup.成功"));
              this.getList();
            }
