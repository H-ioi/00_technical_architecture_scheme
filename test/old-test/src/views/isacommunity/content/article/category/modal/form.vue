...
export default {
  ...
  methods: {
    ...
    // 新增
    addData(data) {
      addContentCategory(data).then((res) => {
        if (res.data.success) {
    ...
    // 编辑
    editData(data) {
      editContentCategory(data).then((res) => {
        if (res.data.success) {
          this.$message.success(this.$t("isagroup.成功"));
          this.$emit("getList");
          this.closeModal();
    ...
    getDetail(id) {
      getContentCategoryDetail(id).then(async (res) => {
        if (res.data.success) {
          let { cnName, enName, index, visible } = res.data.data;
          this.$nextTick(() => {
            this.ruleForm = {
              ...this.ruleForm,
              id,
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          let fromData = {
            ...this.ruleForm,
            visible: this.ruleForm.visible == "1" ? true : false,
          };
