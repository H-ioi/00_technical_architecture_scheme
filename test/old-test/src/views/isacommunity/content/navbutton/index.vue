<template>
  <div class="community_page">
    <div class="community_top">
      <div class="community_top_title">{{ $t("isagroup.导航按钮") }}</div>
    ...
    <div class="community_centent">
      <div class="isa_table">
        <Table
          ref="Table"
          :showSelection="false"
          :tableTitle="tabletitle['navigateButtonTable']"
          :tableData="tableData"
          :tableBtn="tableBtn"
          @playTab="playTab"
          @rowClick="rowClick"
  ...
  methods: {
    ...
    getList() {
      getNavigateButtonPage({
        ...
        if (res.data.success) {
          console.log("getNavigateButtonPage", res.data.data);
    ...
    playTab(name, item, scope) {
      ...
      switch (name) {
        ...
        case "del":
          ...
          delNavigateButton(formData).then((res) => {
            if (res.data.success) {
    ...
    delData() {
      ...
      if (selectionId.length == 0) {
        ...
        this.$alert(
          ...
          delNavigateButton({ ids: selectionId }).then((res) => {
            if (res.data.success) {
              this.$message.success(this.$t("isagroup.成功"));
              this.getList();
            }
