<template>
  <div class="community_page">
    <div class="community_top">
      <div class="community_top_title">{{ $t("isagroup.一周食谱") }}</div>
      <div class="community_top_btn">
        <el-button
          v-if="permissions['foodweekly_add']"
          type="primary"
          size="large"
          @click="showForm('add')"
          >{{ $t("btn.新增") }}</el-button
    ...
    <div class="community_centent">
      ...
      <div class="isa_table">
        <Table
          ref="Table"
          :showSelection="false"
          :tableTitle="tabletitle['foodWeeklyTable']"
          :tableData="tableData"
          :tableBtn="tableBtn"
          :height="520"
          @playTab="playTab"
    ...
    return {
      ...
      permissionsBtn: [
        ...
        // },
        {
          ...
          type: "edit",
          icon: "",
          permissions: "foodweekly_edit",
        },
        {
          name: "删除",
          type: "del",
          icon: "",
          permissions: "foodweekly_del",
        },
      ],
      schoolList: [],
    };
  ...
  methods: {
    ...
    getList() {
      getFoodWeeklyPage({
        ...
        if (res.data.success) {
          console.log("getFoodWeeklyPage", res.data.data);
    ...
    playTab(name, item, scope) {
      ...
      switch (name) {
        ...
        case "del":
          ...
          delFoodWeekly(formData).then((res) => {
            if (res.data.success) {
    ...
    delData() {
      ...
      if (selectionId.length == 0) {
        ...
        this.$alert(
          ...
          delFoodWeekly({ ids: selectionId }).then((res) => {
            if (res.data.success) {
              this.$message.success(this.$t("isagroup.成功"));
              this.getList();
            }
