<template>
  <div class="community_page">
    <div class="community_top">
      <div class="community_top_title">{{ $t("isagroup.学校动态") }}</div>
      <div class="community_top_btn">
        <el-button
          v-if="permissions['schoollife_add']"
          type="primary"
          size="large"
          @click="showForm('add')"
          >{{ $t("btn.新增") }}</el-button
    ...
    <div class="community_centent">
      ...
      <div class="isa_table">
        <Table
          ref="Table"
          :showSelection="false"
          :tableTitle="tabletitle['contentMomentTable']"
          :tableData="tableData"
          :tableBtn="tableBtn"
          :height="520"
          @playTab="playTab"
    ...
    return {
      ...
      permissionsBtn: [
        ...
        // },
        {
          ...
          type: "edit",
          icon: "",
          permissions: "schoollife_edit",
        },
        {
          name: "删除",
          type: "del",
          icon: "",
          permissions: "schoollife_del",
        },
      ],
      schoolList: [],
      momentTypeList: [],
  ...
  methods: {
    ...
    getList() {
      getContentMomentPage({
        ...
        if (res.data.success) {
          console.log("getContentMomentPage", res.data.data);
    ...
    formatData() {
      this.tableData.map((item) => {
        item["sendSmsLabel"] = item["sendSms"]
          ? this.$t("isagroup.是")
          : this.$t("isagroup.否");
        item["pushLabel"] = item["push"]
          ? this.$t("isagroup.是")
          : this.$t("isagroup.否");
        item["pushedLabel"] = item["pushed"]
          ? this.$t("isagroup.是")
          : this.$t("isagroup.否");
    ...
    playTab(name, item, scope) {
      ...
      switch (name) {
        ...
        case "del":
          ...
          delContentMoment(formData).then((res) => {
            if (res.data.success) {
    ...
    delData() {
      ...
      if (selectionId.length == 0) {
        ...
        this.$alert(
          ...
          delContentMoment({ ids: selectionId }).then((res) => {
            if (res.data.success) {
              this.$message.success(this.$t("isagroup.成功"));
              this.getList();
            }
