<template>
  <div class="community_page">
    <div class="community_top">
      <div class="community_top_title">{{ $t("isagroup.点赞收藏") }}</div>
      <div class="community_top_btn">
        <el-button
          v-if="permissions['likeandsave_add']"
          type="primary"
          size="large"
          @click="showForm('add')"
          >{{ $t("btn.新增") }}</el-button
    ...
    <div class="community_centent">
      <div class="isa_table">
        <Table
          ref="Table"
          :showSelection="false"
          :tableTitle="tabletitle['likeAndFavorTable']"
          :tableData="tableData"
          :tableBtn="tableBtn"
          :height="520"
          @playTab="playTab"
...
import {
  getDiscussionLikeAndFavorPage,
  delDiscussionLikeAndFavor,
    ...
    return {
      ...
      permissionsBtn: [
        ...
        // },
        {
          ...
          type: "edit",
          icon: "",
          permissions: "likeandsave_edit",
        },
        {
          name: "删除",
          type: "del",
          icon: "",
          permissions: "likeandsave_del",
        },
      ],
      schoolList: [],
      roleList: [],
  ...
  methods: {
    ...
    getList() {
      getDiscussionLikeAndFavorPage({
        ...
        if (res.data.success) {
          console.log("getDiscussionLikeAndFavorPage", res.data.data);
    ...
    formatData() {
      this.tableData.map((item) => {
        item["favorLabel"] = item["favor"]
          ? this.$t("isagroup.是")
          : this.$t("isagroup.否");
        item["likeLabel"] = item["like"]
          ? this.$t("isagroup.是")
          : this.$t("isagroup.否");
    ...
    playTab(name, item, scope) {
      ...
      switch (name) {
        ...
        case "del":
          ...
          delDiscussionLikeAndFavor(formData).then((res) => {
            if (res.data.success) {
    ...
    delData() {
      ...
      if (selectionId.length == 0) {
        ...
        this.$alert(
          ...
          delDiscussionLikeAndFavor({ ids: selectionId }).then((res) => {
            if (res.data.success) {
              this.$message.success(this.$t("isagroup.成功"));
              this.getList();
            }
