          ...
          <div class="orderDetail_item">
            <div class="orderDetail_baseinfo">
              <div
                ...
                v-for="(item, index) in tabletitle['discussionCommentTable']"
                :key="index"
              >
...
import { getDiscussionCommentDetail } from "@/api/isacommunity/content.js";
  ...
  methods: {
    ...
    getDetail(id) {
      getDiscussionCommentDetail(id).then(async (res) => {
        if (res.data.success) {
          this.$nextTick(() => {
            let { id, discussionId, parentId, content, visible, createdAt } =
              res.data.data;
            this.detailData = {
              ...this.ruleForm,
              id,
