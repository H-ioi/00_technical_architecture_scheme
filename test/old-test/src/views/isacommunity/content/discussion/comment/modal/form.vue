      ...
      <div class="moadlFromBox" v-if="showModal">
        ...
        >
          ...
          >
            ...
            <el-form-item label="评论内容" prop="content" style="width: 100%">
              <el-input
                style="width: 100%"
...
export default {
  ...
  methods: {
    ...
    // 新增
    addData(data) {
      addDiscussionComment(data).then((res) => {
        if (res.data.success) {
    ...
    // 编辑
    editData(data) {
      editDiscussionComment(data).then((res) => {
        if (res.data.success) {
          this.$message.success(this.$t("isagroup.成功"));
          this.$emit("getList");
          this.closeModal();
    ...
    getDetail(id) {
      getDiscussionCommentDetail(id).then(async (res) => {
        if (res.data.success) {
          let { discussionId, parentId, content, visible } = res.data.data;
          this.$nextTick(() => {
            this.ruleForm = {
              ...this.ruleForm,
              id,
