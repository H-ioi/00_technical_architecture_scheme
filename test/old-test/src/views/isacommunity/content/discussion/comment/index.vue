<template>
  <div class="community_page">
    <div class="community_top">
      <div class="community_top_title">{{ $t("isagroup.讨论评论") }}</div>
      <div class="community_top_btn">
        <el-button
          v-if="permissions['comment_add']"
          type="primary"
          size="large"
          @click="showForm('add')"
          >{{ $t("btn.新增") }}</el-button
    ...
    <div class="community_centent">
      <div class="isa_table">
        <Table
          ref="Table"
          :showSelection="false"
          :tableTitle="tabletitle['discussionCommentTable']"
          :tableData="tableData"
          :tableBtn="tableBtn"
          :height="520"
          @playTab="playTab"
  ...
  methods: {
    ...
    getList() {
      getDiscussionCommentPage({
        ...
        if (res.data.success) {
          console.log("getDiscussionCommentPage", res.data.data);
    ...
    playTab(name, item, scope) {
      ...
      switch (name) {
        ...
        case "edit":
          ...
          break;
        case "del":
          delDiscussionComment(item.id).then((res) => {
            if (res.data.success) {
    ...
    delData() {
      ...
      if (selectionId.length == 0) {
        ...
        this.$alert(
          ...
          delDiscussionComment({ ids: selectionId }).then((res) => {
            if (res.data.success) {
              this.$message.success(this.$t("isagroup.成功"));
              this.getList();
            }
