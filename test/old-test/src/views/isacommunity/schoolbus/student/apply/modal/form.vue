          ...
          <div style="max-height: 600px; overflow-y: auto">
            <div class="df_center_wrap">
              ...
              >
                ...
                >
                  <el-option
                    :key="k"
                    v-for="(i, k) in consts['pickupMethod']"
              ...
              <div class="df_center_wrap form_item" style="width: 100%">
                <div class="form_item_top df_sb">
                  ...
                  <el-button
                    v-if="!formDisabled() && routeTableData.length < 2"
                    @click="addRoute"
                    class="button_text"
                    size="small"
                    type="text"
            ...
            >
              ...
              <el-radio-group
                ...
                v-model="ruleForm.approvalStatus"
              >
                <el-radio :label="'1'">{{ $t("isagroup.同意") }}</el-radio>
              </el-radio-group>
            </el-form-item>
            <el-form-item
            ...
            >
              ...
              >
                ...
                >
                  <el-option
                    :key="k"
                    v-for="(i, k) in consts['paymentMethod']"
                    :label="i18nlocel == 'en' ? i.enLabel : i.label"
                    :value="i.value"
                  ></el-option>
                </el-select>
          ...
          <div class="moadlFromBox">
            ...
            >
              ...
              >
                <el-option
                  :key="k"
                  v-for="(i, k) in consts['studentLineType']"
            ...
            >
              <el-select
                style="width: 100%"
                multiple
                v-model="routeForm['ridingWeekDay']"
                :placeholder="$t('common.请选择')"
                :disabled="formDisabled()"
              >
                <el-option
                  :key="k"
                  v-for="(i, k) in ridingWeekDays"
                  :label="i"
                  :value="i"
                ></el-option>
              </el-select>
  ...
  methods: {
          ...
          this.$nextTick(() => {
            ...
            this.routeTableData = orderLines.map((item) => {
              return {
                ...
                ridingDay:
                  !item.ridingStartDay || !item.ridingEndDay
                    ? []
                    : [item.ridingStartDay, item.ridingEndDay],
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        ...
        if (pass && this.routeTableData.length == 0) {
          pass = false;
          this.$message.error(this.$t("isagroup.请选择路线"));
        }
        if (
          pass &&
          this.personTableData.length == 0 &&
          this.ruleForm.pickupMethod == "2"
        ) {
          pass = false;
          this.$message.error(this.$t("isagroup.请选择接送人"));
        }
        // const idPairs = new Set();
        // for (const line of this.routeTableData) {
        //   const pair = `${line.lineId}-${line.stationId}`;
        //   if (idPairs.has(pair)) {
        //     this.$message.error(this.$t("isagroup.存在重复的路线和站点组合"));
        //     return;
        //   }
        //   idPairs.add(pair);
        // }
    ...
    setDefaultDate() {
      if (this.ruleForm.sectionId) {
        const selectedSection = this.selectSectionList.find(
          (section) => section.id === this.ruleForm.sectionId
        );
        if (selectedSection) {
          this.routeForm = {
            ...this.routeForm,
      ...
      this.$nextTick(() => {
        if (this.personForm["pickupImageUrl"]) {
          this.$refs.UploadPersonFile.imageUrl = this.personForm["pickupImageUrl"]
            ? this.personForm["pickupImageUrl"]
            : "";
        } else {
          this.$refs.UploadPersonFile.imageUrl = "";
        }
