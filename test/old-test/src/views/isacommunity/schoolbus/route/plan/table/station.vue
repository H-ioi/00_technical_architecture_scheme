  ...
  <div>
        ...
        <el-row :gutter="10">
          ...
          <el-col :span="6">
            <el-form-item>
              <el-input
                clearable
                v-model="searchFrom.stationName"
                :placeholder="$t('schoolbus.输入站点名称')"
    ...
    <div class="isa_table">
      ...
      <div class="df_sb isa_table_footer">
        <div>
          <el-button
            v-if="permissions['busstation_del']"
    ...
    <StationForm ref="StationForm" @getList="getList" />
    <Detail
      ref="Detail"
    ...
    />
    <BatchStation ref="BatchStation" @getList="getList" />
  </div>
</template>

<script>
import { mapGetters } from "vuex";
import { getStationPage, delStation } from "@/api/isacommunity/station.js";
import Table from "@/components/communitycommon/Table.vue";
import Pagination from "@/components/communitycommon/Pagination.vue";
import StationForm from "../modal/stationform.vue";
...
export default {
  ...
  data() {
    return {
      ...
      permissionsBtn: [
        ...
        { name: "编辑", type: "edit", icon: "", permissions: "busstation_edit" },
      ],
      detailData: {},
      schoolId: "",
    };
  ...
  computed: {
    ...
    tableTitle() {
      return [
        ...
        { label: this.$t("schoolbus.站点"), prop: "showStationName", minWidth: "160" },
  ...
  methods: {
    ...
    getList() {
      getStationPage({ ...this.pagination, ...this.searchFrom }).then((res) => {
        if (res.data.success) {
    ...
    formatData() {
      this.tableData.forEach((item) => {
        ...
        item.showStationName = this.i18nlocel == "en" ? item.enName : item.cnName;
      });
    },
    playTab(name, item) {
    ...
    delData() {
      this.confirmBatchDelete(() => {
        delStation({ ids: this.selectedIds }).then((res) => {
          if (res.data.success) {
    ...
    showForm(type = "add", item = {}) {
      this.$refs.StationForm.showForm(type, item);
    },
    batchUpdload() {
      this.$refs.BatchStation.showUpload = true;
    },
  },
};
</script>
