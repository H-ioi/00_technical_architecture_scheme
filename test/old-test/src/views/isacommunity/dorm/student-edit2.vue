      ...
      <el-row class="detail-row">
        ...
        <el-col :span="9">
          ...
          <span class="detail-value">
            {{ studentInfo.student.gender === '1' || studentInfo.student.gender === 1 ? $t('dorm.男') :
              (studentInfo.student.gender === '2' || studentInfo.student.gender === 2 ? $t('dorm.女') :
    ...
    <div class="detail-section">
      ...
      <el-row class="detail-row">
        <el-col :span="24">
          <div style="display: flex;align-items: center;">
            ...
            <el-date-picker v-model="form.checkoutDate" type="date" :placeholder="$t('dorm.请选择退宿日期')"
              format="yyyy-MM-dd" value-format="yyyy-MM-dd" size="medium" style="width: 250px;"></el-date-picker>
          </div>
      ...
      <el-row class="detail-row">
        <el-col :span="24">
          ...
          <span class="detail-value">{{ studentInfo.student.payment_status === '1' || studentInfo.student.payment_status
            === 1 ? $t('dorm.已缴费') : $t('dorm.未缴费') }}</span>
        </el-col>
      </el-row>
    </div>
...
<script>
import { getStudentHistoryDetail, editStudentHistory } from '@/api/isacommunity/dorm'
...
export default {
        ...
        if (this.detailData && this.detailData.admission_no) {
          getStudentHistoryDetail({ admissionNo: this.detailData.admission_no }).then(res => {
            if (res) {
              ...
              this.form.checkoutDate = res.checkout_date || ''
            }
          }).catch(err => {
  ...
  methods: {
    async submitForm() {
      ...
      let params = {
        admissionNo: this.detailData.admission_no,
        checkoutDate: this.form.checkoutDate
      }
      const res = await editStudentHistory(params);
      if (res) {
        this.$message({
          message: '保存成功',
          type: 'success'
