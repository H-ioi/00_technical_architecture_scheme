            ...
            <template slot-scope="scope" class="df_center">
              <div class="df_align_center table_textbtn">
                <el-button
                  ...
                  :key="b"
                  style="padding: 0"
                  @click.native.prevent="PlayCurrentItem(s, scope.row)"
                  type="text"
                  size="small"
                >
                  <span> {{ s }}</span>
...
import { getEmailList, getEmailPage, updateEmail } from "@/api/base/index.js";
  ...
  methods: {
    ...
    fetchtypelist() {
      fetchTypeList(this.contentType).then((res) => {
        ...
        this.getEmailPage(this.pagination);
      });
    },
    getEmailPage(data) {
      getEmailPage(data).then((res) => {
        console.log("res.data", res.data);
    ...
    changeSchool(item) {
      ...
      this.getEmailPage({ school: this.currentSchoolId });
    },
