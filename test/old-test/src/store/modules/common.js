...
import { fetchTypeList } from "@/api/publik";
import { filterDictFieldAll, filterDictFieldData } from "@/util/filter.js";
    ...
    SET_DICTVALUESDATA: (state, obj) => {
      ...
      if (dataDictValues[obj["type"]]) {
        arr = obj["data"].filter((i) => {
          return (
            dataDictValues[obj["type"]].includes(i.value) &&
            i.status &&
            !i.archived
          );
        });
