 ...
 * 判断路由是否相等
 */
export const isObjectValueEqual_route = (a, b) => {
  let result = true
...
export function createCode() {
  ...
  const random = [1, 2, 3, 4, 5, 6, 7, 8, 9]; //随机数的取值范围
  for (let i = 0; i < codeLength; i++) {
    //循环遍历codeLength，值是几，就循环几次
    let index = Math.floor(Math.random() * 9); //Math.floor方法用于返回小于或等于一个给定数字的最大整数；Math.random方法返回 0（含）和 1（不含）之间的随机数
