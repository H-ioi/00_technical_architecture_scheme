...
export const MODULE_OPTIONS = [
  ...
  { id: 3, label: '宿舍', enLabel: 'Dormitory' }
]
...
export function normalizeSchoolIdValues(raw) {
  ...
  if (Array.isArray(raw)) {
    ...
    return raw.filter((id) => id != null && id !== '').map((id) => String(id))
  }
  return [String(raw)]
}
...
export function formatOptionLabels(ids, options, i18nlocel = 'zh') {
  ...
  const labelList = options.filter((item) => idSet.includes(Number(item.id))).map((item) => (i18nlocel === 'en' ? item.enLabel : item.label))
...
export function formatSchoolLabels(schoolIds, schoolDictionary, getListLabel) {
  ...
  const labels = ids.map((id) => getListLabel(schoolDictionary, id, 'enName', 'id')).filter((l) => l && l !== '--')
