      ...
      >
        <el-scrollbar class="formItem">
          ...
          <div class="df_center_wrap">
            ...
            <el-form-item label="巡检类型" prop="carryTime" v-if="from.school">
              <el-select
                style="width: 100%"
                v-model="from.inspectType"
                placeholder="请选择"
                prop="inspectType"
              >
                <el-option
                  :key="k"
                  v-for="(i, k) in orderTypeList['order_inspect_type']"
                  :label="i.label"
                  :value="i.value"
                ></el-option>
              </el-select>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      rules: {
        ...
        inspectType: [{ required: true, message: "请选择", trigger: "blur" }]
      },
      currentOrderId: "",
      orderTypeList: [],
      isMy: null
  ...
  methods: {
    ...
    // 编辑工单
    putOrder(data) {
      putOrderInspect(data).then(res => {
        if (res.data.success) {
          this.$message.success("编辑成功");
          this.$store.commit("CLOSE_TAG_CURRENT", this.$route.fullPath);
          this.$router.push(
            `/order/inspect/detail?id=${data.orderId}&isMy=${this.isMy}`
          );
        }
      });
    },
    ...
    setData(res, id) {
      if (res.data.success) {
        ...
        this.from = {
          ...
          department,
          createTime,
          inspectType: orderInspect["inspectType"],
          spaceId: orderInspect["spaceId"],
          assetId: orderInspect["assetId"],
          description: orderInspect["description"]
        };
        this.$nextTick(() => {
          this.$refs["SpaceTree"].labelModel = spaceName;
          this.$refs["SpaceTree"].currentNodeKey = orderInspect["spaceId"];
          this.$refs["SpaceTree"].defaultCheckedKeys = orderInspect["spaceId"]
            ? [orderInspect["spaceId"]]
            : [];
          this.$refs["AssetList"].assetStr = assetName;
          this.$refs["AssetList"].assetId = orderInspect["assetId"];
