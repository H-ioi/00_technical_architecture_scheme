      ...
      >
        <el-scrollbar class="formItem">
          ...
          <div class="df_center_wrap">
            ...
            <el-form-item label="维修类型" prop="repairType" v-if="from.school">
              <el-select
                style="width: 100%"
                v-model="from.repairType"
                placeholder="请选择"
                prop="repairType"
              >
                <el-option
                  :key="k"
                  v-for="(i, k) in orderTypeList['order_repair_type']"
                  :label="i.label"
                  :value="i.value"
                ></el-option>
              </el-select>
  ...
  data() {
    ...
    return {
      ...
      rules: {
        ...
        repairType: [{ required: true, message: "请选择", trigger: "blur" }]
      },
      currentOrderId: "",
      orderTypeList: [],
      isMy: null
    ...
    setData(res, id) {
      if (res.data.success) {
        ...
        this.from = {
          ...
          department,
          createTime,
          repairType: orderRepair["repairType"],
          cost: orderRepair["cost"],
          spaceId: orderRepair["spaceId"],
          assetId: orderRepair["assetId"],
          description: orderRepair["description"]
        };
        this.$nextTick(() => {
          this.$refs["SpaceTree"].labelModel = spaceName;
          this.$refs["SpaceTree"].currentNodeKey = orderRepair["spaceId"];
          this.$refs["SpaceTree"].defaultCheckedKeys = orderRepair["spaceId"]
            ? [orderRepair["spaceId"]]
            : [];
          this.$refs["AssetList"].assetStr = assetName;
          this.$refs["AssetList"].assetId = orderRepair["assetId"];
