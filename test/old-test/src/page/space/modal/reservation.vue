      ...
      <div class="moadlFromBox">
        ...
        >
          ...
          <div class="df_center_wrap" v-else>
            <el-form-item label="预定名称" style="width: 100%" prop="name">
              <el-input
            ...
            <el-form-item label="预定时间" style="width: 100%" prop="time">
              <div class="df_sb">
            ...
            <el-form-item label="预定人" style="width: 100%" prop="username">
              <el-input
            ...
            <el-form-item label="预定人电话" style="width: 100%" prop="phone">
              <el-input
...
export default {
  ...
  methods: {
    // 新增预定
    addReserveSpace(data) {
      addReserveSpace(data).then((res) => {
        if (res.data.success) {
          this.$message.success("新增预定成功");
    ...
    // 编辑预定
    putSpaceReserve(data) {
      putSpaceReserve(data).then((res) => {
        if (res.data.success) {
    ...
    delSpaceReserve(id) {
      delSpaceReserve(id).then((res) => {
        if (res.data.success) {
          ...
          this.$emit("delSpaceReserve", id);
          this.closeModal();
        }
      });
    },
    delReserve() {
      this.$alert("确认删除吗", "删除预定", {
        confirmButtonText: "确定",
      }).then(() => {
        this.delSpaceReserve(this.ruleForm.id);
      });
    },
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          if (this.reservationType == "add") {
            ...
            this.addReserveSpace(data);
          } else if (this.reservationType == "edit") {
            this.putSpaceReserve(data);
          }
        } else {
          return false;
        }
