...
import { bindSpaceLabel } from "@/api/space/spacelabel.js";
...
export default {
  ...
  data() {
    return {
      ...
      tableTitle: [
        ...
        { label: "绑定标签", prop: "typeFormsName", width: "" },
      ],
      // 所有
      tableData: [],
      tableBtn: [],
  ...
  methods: {
    ...
    bindSpaceLabel(id, obj) {
      console.log("bindSpaceLabel", id, obj);
      let spaceIdsStr = "";
      if (obj["spaceIds"].length > 0) {
        obj["spaceIds"].map((item, index) => {
          spaceIdsStr =
            spaceIdsStr + (index == 0 ? "?" : "&") + "spaceIds=" + item;
        });
      }
      console.log("spaceIdsStr", spaceIdsStr);
      // return;
      bindSpaceLabel(id, spaceIdsStr).then((res) => {
        if (res.data.success) {
    ...
    handleOk() {
      ...
      this.bindSpaceLabel(this.currentSpace.id, obj);
      // }
    },
    // 解绑
    unBind() {
      ...
      data.map((item, index) => {
        if (item.status === 1) {
          ...
          if (item.labels == null) {
            ...
            item.labels.map((i) => {
              typeFormsName.push(i.name);
              typeFormsId.push(i.id);
            });
            item = Object.assign(item, {
              typeFormsName,
              typeFormsId,
    ...
    setTreeTableData(data, arr) {
      console.log("setTreeTableData", data);
      data.map((item) => {
        if (item.status === 1) {
          arr.push(item);
          if (item.children) {
