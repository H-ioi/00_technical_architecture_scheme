        ...
        >
          <div>
            <el-form-item
              :ref="item.fontId"
              :id="item.fontId"
              style="width: 100%; padding: 0 0 20px !important"
              :class="[
                'list-complete-item',
                {
            ...
            >
              ...
              />
              <p
                style="line-height: 20px !important; padding: 5px 0; margin: 0"
...
export default {
  ...
  created() {
    this.$nextTick(() => {
      console.log("getTemplateDetail", this.template, this.childTemplateId);
  ...
  methods: {
    //获取动态表单模板详情
    getTemplateDetail() {
      console.log("getTemplateDetail", this.childTemplateId);
      if (this.childTemplateId == null) return;
      getTemplateInfo({
        templateId: this.childTemplateId,
        fontId: this.template.fieldId,
      }).then((res) => {
        if (res.data.success) {
          this.templateData.templateFormId = this.childTemplateId;
          this.setform = {};
          this.formArr = [];
