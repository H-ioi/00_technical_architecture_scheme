<template>
  <el-scrollbar class="formgenerator_form_right">
    <div class="formgenerator_form_right_title">
      {{ $t("consult.页面布局预览") }}
    </div>
    ...
    <div class="draggable">
      <el-form
        ...
        ref="formArr"
      >
        <draggable class="list-group" :list="formArr" group="people">
          <transition-group tag="div" name="list-complete">
            ...
            >
              ...
              >
                ...
                >
                  <el-radio
                    v-for="(i, k) in item.properties.option"
                    :key="k"
                    :disabled="item.disabled"
                    :readonly="item.readonly"
                    :label="i.id"
        ...
        <el-form-item class="editFromBtn">
          <el-button
            ...
            size="medium"
            round
            @click="submitForm('formArr')"
            >{{ $t("consult.提交") }}</el-button
          >
        </el-form-item>
      </el-form>
    </div>
...
export default {
  ...
  methods: {
    ...
    // 删除
    delThisForm(index) {
      this.$emit("delThisForm", index);
    },
    // 提交
    submitForm(formName) {
      this.$nextTick(() => {
        console.log("templateRule", this.formArrRules);
