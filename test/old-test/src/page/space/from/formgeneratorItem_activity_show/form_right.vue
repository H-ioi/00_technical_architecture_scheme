<template>
  <el-scrollbar class="formgenerator_form_right">
    ...
    <div class="draggable" style="padding: 0 20px 0 0 !important">
      <el-form
        :label-position="'top'"
        :inline="true"
        :model="formArrValue"
          ...
          <transition-group tag="div" name="list-complete">
            ...
            >
              <div>
                <div
                  class="formItemBtn"
                  v-if="setform.fontId == item.fontId"
                ></div>
                <el-form-item
                  ...
                  :class="[
                    ...
                    {
                      is_showHidden: item.showHidden,
                    },
                    {
                      'is-title': item.type == 'title',
                    },
                  ]"
                  :label="item.label + (item.showHidden ? '(隐藏)' : '(显示)')"
                  :prop="item.fontId"
                >
                  <!-- 单文本输入框 -->
                  <el-input
...
import ChildForm from "../formgeneratorItem_activity/child_form.vue";
export default {
  components: {
    draggable,
    ChildForm,
  ...
  methods: {
    ...
    getCurrentItemForm(data) {
      let typeList = ["checkbox", "radio", "select"];
      if (!typeList.includes(data.type)) return;
    ...
    resetFields(event, item) {
      this.$emit("resetFields", event, item);
    },
  },
};
</script>
