...
import { fetchMessageUnread } from "@/api/message/index.js";
import { getEmsUserInfo } from "@/api/ems/index";
import { clearAllPendingRequests } from '@/router/repeat-request'
...
const user = {
	...
	actions: {
		// 根据微软授权登录
		LoginByMicrosoft ({ commit }, data) {
			return new Promise((resolve, reject) => {
				getEmsUserInfo(data).then(response => {
					console.log('getEmsUserInfo', response);
					const data = response.data.data
					commit('SET_ACCESS_TOKEN', data.access_token)
					commit('SET_REFRESH_TOKEN', data.refresh_token)
					commit('SET_EXPIRES_IN', data.expires_in)
					commit('SET_USER_INFO', data.userInfo)
					commit('SET_PERMISSIONS', data.userInfo.authorities || [])
					commit('CLEAR_LOCK')
					commit('DEL_ALL_TAG')
					resolve()
				}).catch(error => {
		...
		FetchMessageUnread ({ commit }) {
			return new Promise((resolve, reject) => {
				fetchMessageUnread().then(res => {
					let data = res.data.data;
					let messageNum = {}
					let unreadnum = []
					let num = 0;
					Object.keys(data).forEach((item) => {
						num = num + Number(data[item]);
						let obj = {
							name: item,
							num: data[item],
						};
					...
					messageNum['unreadall'] = num
					messageNum['unreadnum'] = unreadnum
					commit('SET_MESSAGENUM', messageNum)
					resolve(messageNum)
				}).catch(() => {
					reject()
				})
