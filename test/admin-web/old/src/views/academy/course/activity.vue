  ...
  <div>
    <div class="searchFromBox">
      <div class="df_sb searchFromBox_header">
        <div class="searchFromBox_header_titel">活动管理</div>
      </div>
    ...
    <div class="df_sb palyTableBox">
      <div>
        <el-button
          v-if="permissions['isaic_activity_add']"
          type="primary"
          size="medium"
          @click="$router.push('/academy/course/editactivity?type=add')"
          >添加活动</el-button
        >
        ...
        >
          <template slot-scope="scope">
            <div class="df_align_center table_textbtn">
              ...
              <span v-if="permissions['isaic_activity_edit']">
                <el-button
              ...
              <span
                v-if="
                  permissions['isaic_activity_release'] &&
              ...
              <span v-if="permissions['isaic_activity_download_signup']">
                <el-button
                  type="text"
                  size="small"
              ...
              <span v-if="permissions['isaic_activity_del']">
                <el-button
                  :disabled="scope.row.release == 1"
                  type="text"
                  size="small"
...
import CourseForm from "./formlist/activityform.vue";
...
import {
  ...
  upActivity,
  delActivity,
  downloadActivitySignup
} from "@/api/academy/activity.js";
  ...
  methods: {
    getTabelData() {
      ...
      getActivityPage(data).then(res => {
        console.log("getActivityPage", res);
        if (res.data.success) {
    ...
    playTab(type, row, scope) {
      switch (type) {
        case "look":
          this.$router.push(`/academy/course/coursedetail?type=2&id=${row.id}`);
          break;
        case "edit":
          this.$router.push(
            "/academy/course/editactivity?type=edit&id=" + row.id
          );
          break;
        case "up":
        ...
        case "down":
          ...
          break;
        case "download":
          downloadActivitySignup(row["id"]).then(res => {
            this.$message({
              type: "success",
              message: "下载成功"
            });
