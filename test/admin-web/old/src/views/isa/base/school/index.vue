...
export default {
  ...
  data() {
    return {
      ...
      treeData: [
        {
          id: "isadatacenter_enroll_level",
          label: "年级",
          type: "isadatacenter_enroll_level",
          pid: null,
          sort: null,
          status: null,
          value: null,
      ...
      dictTypeList: [],
      dictTypeObj: {
        isadatacenter_enroll_level: "年级",
      },
      dictTypeIds: [],
      dictTypeIdsObj: {},
      typeFile: {
        isadatacenter_enroll_level: false,
      },
      isAll: {
        isadatacenter_enroll_level: false,
      },
      dictTypes: [
        "isadatacenter_enroll_level",
      ]
    };
  },
  created() {
  ...
  methods: {
    ...
    getRelationDictChild() {
      getRelationDictChild(this.currentid).then(res => {
        console.log("getRelationDictChild", res);
        if (res.data.data) {
          this.cascaderArr = [];
          this.$nextTick(() => {
    ...
    setIsAll() {
      ...
      Object.keys(this.dictTypeIdsObj).forEach(res => {
        if (this.dictTypeIdsObj[res].length == 0) {
          ...
          if (
            ids.length != 0 &&
            ids.length == this.dictTypeIdsObj[res].length
          ) {
            this.$nextTick(() => {
              this.isAll[res] = true;
            });
