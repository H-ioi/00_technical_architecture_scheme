<template>
  <div class="community_page">
    <div class="community_top">
      <div class="community_top_title">{{ $t("isagroup.司机管理") }}</div>
    ...
    <div class="community_centent">
      ...
      <div class="isa_table">
        <Table
          ref="Table"
          :showSelection="true"
          :tableTitle="tabletitle['busdriverTable']"
          :tableData="tableData"
          :tableBtn="tableBtn"
          @playTab="playTab"
          @rowClick="rowClick"
  ...
  methods: {
    ...
    getList() {
      getBusdriverPage({
        ...
        if (res.data.success) {
          console.log("getBusdriverPage", res.data.data);
    ...
    formatData() {
      this.tableData.map((item) => {
        item["statusLabel"] = this.$getListLabel(consts["serviceType"], item["status"]);
    ...
    delData() {
      ...
      if (selectionId.length == 0) {
        ...
        this.$alert(this.$t("isagroup.确定要删除吗？"), this.$t("isagroup.删除"), {
          ...
          delBusdriver({ ids: selectionId }).then((res) => {
            if (res.data.success) {
