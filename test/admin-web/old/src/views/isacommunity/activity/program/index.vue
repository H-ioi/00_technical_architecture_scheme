<template>
  <div class="community_page">
    <div class="community_top">
      <div class="community_top_title">{{ $t("isagroup.活动项目") }}</div>
    ...
    <div class="community_centent">
      ...
      <div class="isa_table">
        <Table ref="Table" :showSelection="true" :tableTitle="tabletitle['activityProgramTable']" :tableData="tableData"
...
import Copy from "./modal/copy.vue";
import dayjs from "dayjs";
export default {
  name: "teacher",
  components: { Table, Pagination, Form, Copy },
  data() {
    return {
      consts: consts,
  ...
  methods: {
    ...
    getList() {
      getActivityProgramPage({
        ...
        if (res.data.success) {
          console.log("getActivityProgramPage", res.data.data);
    ...
    formatData() {
      this.tableData.map((item) => {
        item["programStatusLabel"] = this.$getListLabel(
          consts["programStatus"],
          item["programStatus"]
        );
        item["programTypeLabel"] = this.$getListLabel(
          consts["programType"],
          item["programType"]
        );
    ...
    delData() {
      ...
      if (selectionId.length == 0) {
        ...
        this.$alert(this.$t("isagroup.确定要删除吗？"), this.$t("isagroup.删除"), {
          ...
          delActivityProgram({ ids: selectionId }).then((res) => {
            if (res.data.success) {
