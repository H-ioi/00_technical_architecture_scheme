  ...
  <div class="isa_center_page">
    <div class="searchFromBox">
      ...
      >
        ...
        <el-form-item style="width: 180px" :label="$t('isagroup.年级')">
          <el-select
            clearable
            style="width: 100%"
            multiple
        ...
        <el-form-item style="width: 180px" :label="$t('isagroup.班级')">
          <el-select
            clearable
            style="width: 100%"
            multiple
        ...
        <el-form-item style="width: 180px" :label="$t('isagroup.住宿')">
          <el-select
            clearable
            style="width: 100%"
            v-model="searchFrom['dormitoryStatus']"
            :placeholder="$t('common.请选择')"
          >
            <el-option
              :key="k"
        ...
        <el-form-item style="width: 180px" :label="$t('isagroup.校巴')">
          <el-select
            clearable
            style="width: 100%"
            v-model="searchFrom['busStatus']"
            :placeholder="$t('common.请选择')"
          >
            <el-option
              :key="k"
        ...
        <el-form-item style="width: 180px" :label="$t('isagroup.状态')">
          ...
          >
            <el-option
              :key="k"
              v-for="(i, k) in consts['studentStatus']"
              :label="i.label"
              :value="i.value"
            ></el-option>
          </el-select>
    ...
    <div class="tablelist">
      <Table
        ref="Table"
        :showSelection="false"
        :tableTitle="tabletitle['studentTable']"
        :tableData="tableData"
        :tableBtn="tableBtn"
        @playTab="playTab"
        @rowClick="rowClick"
    ...
    <Detail
      ref="Detail"
      :title="$t('isagroup.学生详情')"
      :detailInfo="tabletitle['studentInfo']"
  ...
  methods: {
    ...
    getList() {
      getStudentPage({
        ...
        if (res.data.success) {
          console.log("getStudentPage", res.data.data);
          let { data, total, current } = res.data.data;
          this.paginationTotal = total;
          this.tableData = data;
          this.tableData.map((item) => {
            item["dormitoryStatus"] = this.$getListLabel(
              consts["isOrNo"],
              item["dormitoryStatus"]
            );
            item["busStatus"] = this.$getListLabel(consts["isOrNo"], item["busStatus"]);
            item["studentStatus"] = this.$getListLabel(
              consts["studentStatus"],
              item["studentStatus"]
            );
          });
        }
