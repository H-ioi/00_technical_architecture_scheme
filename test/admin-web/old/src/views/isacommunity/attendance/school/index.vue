<template>
  <div class="community_page">
    <div class="community_top">
      <div class="community_top_title">{{ $t("isagroup.校园考勤") }}</div>
      <div class="community_top_btn">
        <el-button
          v-if="permissions['school_attendance_export']"
          type="primary"
          size="large"
          @click="exportData"
          >{{ $t("btn.导出") }}</el-button
    ...
    <div class="community_centent">
        ...
        >
          ...
          <el-form-item style="width: 180px">
            <el-input
              clearable
              style="width: 100%"
              v-model="searchFrom['entryAcsChannel']"
              :placeholder="$t('isagroup.进入通道')"
            ></el-input>
          </el-form-item>
          <el-form-item style="width: 180px">
            <el-input
              clearable
              style="width: 100%"
              v-model="searchFrom['leavingAcsChannel']"
              :placeholder="$t('isagroup.离开通道')"
            ></el-input>
          </el-form-item>
          <el-form-item style="width: 180px">
            <el-date-picker
      ...
      <div class="isa_table">
        <Table
          ref="Table"
          :showSelection="false"
          :tableTitle="tabletitle['schoolAttendanceTable']"
          :tableData="tableData"
          :tableBtn="tableBtn"
          :height="520"
          @playTab="playTab"
  ...
  methods: {
    ...
    formatData() {
      ...
      this.tableData.map((item) => {
        ...
        item["entryOpenType"] = this.$getListLabel(
          consts["attendanceOpenType"],
          item["entryOpenType"]
        );
        item["leavingOpenType"] = this.$getListLabel(
          consts["attendanceOpenType"],
          item["leavingOpenType"]
        );
        item["attendanceDate"] = dayjs(item["attendanceDate"]).format("YYYY-MM-DD");
      });
    },
    exportData() {
      exportSchoolAttendance(this.searchFrom).then((res) => {
