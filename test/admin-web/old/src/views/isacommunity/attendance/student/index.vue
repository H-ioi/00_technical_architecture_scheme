<template>
  <div class="community_page">
    <div class="community_top">
      <div class="community_top_title">{{ $t("isagroup.学生考勤") }}</div>
      <div class="community_top_btn">
        <el-button
          v-if="permissions['student_attendance_export']"
          type="primary"
          size="large"
          @click="exportData"
          >{{ $t("btn.导出") }}</el-button
    ...
    <div class="community_centent">
        ...
        >
          ...
          <el-form-item style="width: 180px">
            <el-select
              ...
              clearable
              style="width: 100%"
              v-model="searchFrom['grade']"
              :placeholder="$t('isagroup.年级')"
            >
              <el-option
                :key="k"
                v-for="(i, k) in gradeList"
                :label="i"
                :value="i"
              ></el-option>
            </el-select>
          ...
          <el-form-item style="width: 180px">
            <el-select
              clearable
              style="width: 100%"
              v-model="searchFrom['onBoarding']"
              :placeholder="$t('isagroup.是否住宿')"
            >
              <el-option
                :key="k"
          ...
          <el-form-item style="width: 180px">
            <el-select
              clearable
              style="width: 100%"
              v-model="searchFrom['onBus']"
              :placeholder="$t('isagroup.是否坐校巴')"
            >
              <el-option
                :key="k"
                v-for="(i, k) in consts['yesOrno']"
          ...
          <el-form-item style="width: 180px">
            <el-select
              clearable
              style="width: 100%"
              v-model="searchFrom['schoolStatus']"
              :placeholder="$t('isagroup.状态')"
            >
              <el-option
                :key="k"
                v-for="(i, k) in consts['attendanceSchoolType']"
                :label="i18nlocel == 'en' ? i.enLabel : i.label"
                :value="i.id"
              ></el-option>
            </el-select>
      ...
      <div class="isa_table">
        <Table
          ref="Table"
          :showSelection="true"
          :tableTitle="tabletitle['studentAttendanceTable']"
          :tableData="tableData"
          :tableBtn="tableBtn"
          :height="520"
          @playTab="playTab"
...
export default {
  ...
  async created() {
    this.gradeList = await getGradeList();
    this.getList();
  },
  mounted() {},
  activated() {
  ...
  methods: {
    ...
    formatData() {
      ...
      this.tableData.map((item) => {
        item["schoolStatus"] = this.$getListLabel(
          consts["attendanceSchoolType"],
          String(item["schoolStatus"])
        );
        item["boarding"] = this.$getListLabel(
          consts["yesOrno"],
          String(item["boarding"])
        );
        item["schoolBus"] = this.$getListLabel(
          consts["yesOrno"],
          String(item["schoolBus"])
        );
        item["attendanceDate"] = dayjs(item["attendanceDate"]).format("YYYY-MM-DD");
      });
    },
    exportData() {
      exportAttendanceStudent(this.searchFrom).then((res) => {
