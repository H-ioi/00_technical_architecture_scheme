...
export default {
  ...
  methods: {
    getAssetTypeTree() {
      getAssetTypeTree({ level: 3 }).then((res) => {
        if (res.data.success) {
    ...
    getAssetTypeMiddle() {
      ...
      getAssetTypeMiddle(this.pagination)
        .then((res) => {
          if (res.data.success) {
    ...
    toformgenerator(type, item) {
      if (type == "add") {
        this.$router.push(
          `/assets/formgenerator?type=add&level=3&menuTypeId=${this.pagination["menuTypeId"]}`
        );
      } else {
        this.$router.push(
          `/assets/formgenerator?type=edit&level=3&menuTypeId=${this.pagination["menuTypeId"]}&assetTypeId=${item.id}`
        );
      }
    },
  },
