...
export default {
  ...
  methods: {
    getAssetTypeTree() {
      getAssetTypeTree({ level: 2 }).then((res) => {
        if (res.data.success) {
    ...
    getAssetTypeLarge() {
      ...
      getAssetTypeLarge(this.pagination)
        .then((res) => {
          if (res.data.success) {
    ...
    toformgenerator(type, item) {
      if (type == "add") {
        this.$router.push(
          `/assets/formgenerator?type=add&level=2&menuTypeId=${this.pagination["menuTypeId"]}`
        );
      } else {
        this.$router.push(
          `/assets/formgenerator?type=edit&level=2&menuTypeId=${this.pagination["menuTypeId"]}&assetTypeId=${item.id}`
        );
      }
    },
  },
