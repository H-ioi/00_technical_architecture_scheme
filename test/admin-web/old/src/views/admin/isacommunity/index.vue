          ...
          <avue-crud ref="crud" :option="option" v-model="form" :page.sync="page" :table-loading="listLoading"
            ...
            <template slot="menuLeft">
              <div class="df_sb">
                ...
                <el-upload class="upload-demo" :show-file-list="false" action="" accept=".xlsx,.xls" :limit="1"
            ...
            <template slot="roleForm" slot-scope="scope">
              <avue-select v-model="role" :dic="rolesOptions" :props="roleProps" multiple filterable
                placeholder="请选择角色" />
            </template>
            <template slot="superiorIdForm" slot-scope="scope">
              <avue-select filterable v-model="superiorId" :dic="userList" :props="levelProps" placeholder="请选择上下级" />
            </template>
          </avue-crud>
        </el-col>
      </el-row>
...
<script>
import { addObj, delObj, fetchList, putObj, importObj, downloadTemplate } from "@/api/admin/user";
import { deptRoleList } from "@/api/admin/role";
import { fetchTree } from "@/api/admin/dept";
import { tableOption } from "@/const/crud/admin/isauser";
...
export default {
  ...
  computed: {
    ...mapGetters(["permissions", "userList"]),
  },
  watch: {
    role() {
    ...
    superiorId() {
      this.form.superiorId = this.superiorId;
    },
  },
  created() {
  ...
  methods: {
    ...
    handleUpdate(row, index) {
      ...
      this.superiorId = row.superiorUser ? row.superiorUser.userId : undefined;
