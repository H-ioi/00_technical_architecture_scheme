<template>
  <div>
    <el-dialog
      :title="title[spaceusageType]"
      :visible.sync="showSpaceusage"
      width="480px"
    ...
    >
      <div class="moadlFromBox">
        <el-form
          ...
          ref="ruleForm"
        >
          <div class="df_center_wrap" v-if="spaceusageType == 'look'">
            <div class="info">
              <div class="info_label">空间名称</div>
              <div class="info_value">
                {{ ruleForm.spaceName ? ruleForm.spaceName : "--" }}
              </div>
            </div>
            <div class="info">
            ...
            <div class="info">
              ...
              <div class="info_value">
                ...
                >&nbsp;至&nbsp;<span>{{ ruleForm.endTime }}</span>
              </div>
            </div>
            <div class="info">
              <div class="info_label">填写人</div>
              <div class="info_value">{{ ruleForm.enterUserName }}</div>
            </div>
            <div class="info">
              <div class="info_label">备注信息</div>
          ...
          <el-form-item
            ...
            v-if="spaceusageType == 'look'"
          >
            <el-button
              v-if="permissions['space_usage_edit']"
              type="primary"
              size="medium"
              @click="editUse"
              >编辑</el-button
            >
            <el-button
              v-if="permissions['space_usage_del']"
              type="default"
              size="medium"
              @click="delusage"
              >删除</el-button
          ...
          <el-form-item v-else class="modalFromBtn" style="margin-top: 20px">
            <el-button
...
export default {
  ...
  mounted() {},
  computed: {
    ...mapGetters(["userList", "userInfo", "permissions"])
  },
  methods: {
    // 删除使用
    delSpaceUsage(id) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          let data = {
            ...
            enterUserId: this.userInfo.id,
            spaceId: this.ruleForm.spaceId
              ? this.ruleForm.spaceId
              : this.$route.query.id
          };
          console.log("data", data);
          if (this.spaceusageType == "add") {
            this.addUsageSpace(data);
          } else if (this.spaceusageType == "edit") {
