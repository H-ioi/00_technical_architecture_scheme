      ...
      >
        ...
        >
          ...
          <el-form-item :label="$t('consult.选项配置')" prop="option">
            ...
            <span class="addoption" @click="addoption">{{ $t("consult.添加选项") }}</span>
          </el-form-item>
        </div>
  ...
  methods: {
    async getMappingFieldInfo() {
      ...
      if (this.queryInfo["scene"] == "1") {
        let list = await getMappingFieldInfo(1);
        list.forEach((item) => {
          if (item["schoolId"] == this.queryInfo["typeId"]) {
            const infoList = item["infoList"];
            infoList.map((info) => {
              if (info["type"] == 1) {
                this.mappingFieldInfo = [...this.mappingFieldInfo, ...info["fieldList"]];
              }
            });
          }
        });
