  ...
  <div class="login-container logingroup">
    <div class="login-weaper animated bounceInDown df_sb">
      <div class="login-black df_center">
        <img style="width: 300px" src="/img/ems/logo.png" alt="" />
      </div>
        ...
        <div class="login-border">
          <div class="login-main">
            ...
            <div class="login-menu">
              ...
              <a :href="`${microsoftPath}/auth/microsoft/auth/render`">微软授权登录</a>
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  beforeRouteEnter(to, from, next) {
    ...
    next((vm) => {
      vm.$nextTick(() => {
        document.title = "ISA EMS";
        vm.setTheme(6);
      });
      if (toQuery["accessToken"] || fromQuery["accessToken"]) {
        vm.loginByMicrosoft({
          accessToken: toQuery["accessToken"] || fromQuery["accessToken"],
        });
      }
      if (toQuery["errorMsg"]) {
        vm.showErrorMsg(toQuery["errorMsg"]);
      }
    });
  },
  data() {
    return {
      ...
      activeName: "user",
      socialForm: {},
      microsoftPath: process.env.VUE_APP_MICROSOFT,
    };
  },
  watch: {
    $route: {
  ...
  methods: {
    loginByMicrosoft(data) {
      ...
      this.$store.dispatch("LoginByMicrosoft", data).then(async () => {
        setTimeout(() => {
          this.$store.commit("DEL_ALL_TAG", true);
          this.$router.push({ path: "/ems/home/index" });
          loading.close();
        }, 300);
      });
    },
