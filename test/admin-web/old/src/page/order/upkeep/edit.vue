      ...
      >
        <el-scrollbar class="formItem">
          ...
          <div class="df_center_wrap">
            ...
            <el-form-item label="保养类型" prop="upkeepType" v-if="from.school">
              <el-select
                style="width: 100%"
                v-model="from.upkeepType"
                placeholder="请选择"
                prop="upkeepType"
              >
                <el-option
                  :key="k"
                  v-for="(i, k) in orderTypeList['order_upkeep_type']"
                  :label="i.label"
                  :value="i.value"
                ></el-option>
              </el-select>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      rules: {
        ...
        upkeepType: [{ required: true, message: "请选择", trigger: "blur" }]
      },
      currentOrderId: "",
      orderTypeList: [],
      isMy: null
  ...
  methods: {
    ...
    // 编辑工单
    putOrder(data) {
      putOrderUpkeep(data).then(res => {
        if (res.data.success) {
          this.$message.success("编辑成功");
          this.$store.commit("CLOSE_TAG_CURRENT", this.$route.fullPath);
          this.$router.push(
            `/order/upkeep/detail?id=${data.orderId}&isMy=${this.isMy}`
          );
        }
      });
    },
    ...
    setData(res, id) {
      if (res.data.success) {
        ...
        this.from = {
          ...
          department,
          createTime,
          upkeepType: orderUpkeep["upkeepType"],
          cost: orderUpkeep["cost"],
          spaceId: orderUpkeep["spaceId"],
          assetId: orderUpkeep["assetId"],
          description: orderUpkeep["description"]
        };
        this.$nextTick(() => {
          this.$refs["SpaceTree"].labelModel = spaceName;
          this.$refs["SpaceTree"].currentNodeKey = orderUpkeep["spaceId"];
          this.$refs["SpaceTree"].defaultCheckedKeys = orderUpkeep["spaceId"]
            ? [orderUpkeep["spaceId"]]
            : [];
          this.$refs["AssetList"].assetStr = assetName;
          this.$refs["AssetList"].assetId = orderUpkeep["assetId"];
