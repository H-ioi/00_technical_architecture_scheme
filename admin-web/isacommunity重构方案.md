...
3. **迭代节奏与当前轮次**  
   ...
   - **下一轮执行方式**：认领下一批菜单/页面 → **先在 `test/old-test` 中定位对应旧页与调用链并完成盘点** → 再在 **`admin-web`** 补齐类型、API、`views`、i18n、路由与映射；继续在 **Phase 3 残留**、**Phase 4～** 滚动推进。**Phase 5 之后默认顺序**：权限 → 考勤 → 内容 → 群发邮件 → **活动管理（置于 Phase 9、收尾之前）**。**严禁跳步、严禁无旧代码依据的新能力**。
...
| P7 | 内容管理 | 公告、动态内容、文章、讨论管理 | 完成内容运营模块 |
| P8 | 群发邮件 | 群组配置、发件箱配置、发件列表 | 完成邮件触达模块 |
| P9 | 活动管理 | 活动列表、活动项目、奖品、投票节目、问卷、家长学生关联、微信配置、邮箱配置 | 完成活动运营链路 |
...
| P6-P9 | 按阶段顺序滚动推进（活动置末） | 默认依次为考勤、内容、邮件；活动管理链路复杂，**排在 P9、收尾 Phase 10 之前**；若上线急需可单列插单，仍以旧代码对齐为前提 |
...
- 常用入口，如成员管理、校车管理及后续已授权业务模块（实施顺序上以考勤、内容、邮件优先，活动置于末段）。
- 权限演示逻辑移除或改为真实权限控制。
...
全量重构按“基座优先、低风险 CRUD 先行、复杂流程后置”的节奏推进。**活动管理**（多 tab、批量微信、多导出等）统一排在**考勤、内容、群发邮件之后**，作为 Phase 9，再行收尾。
...
| Phase 7 | 内容管理 | 公告、动态内容、文章、讨论管理 | 迁移内容发布和互动模块 |
| Phase 8 | 群发邮件 | 群组配置、发件箱配置、发件列表 | 迁移消息触达模块 |
| Phase 9 | 活动管理 | 活动列表、活动项目、奖品、投票节目、问卷、家长学生关联、微信配置、邮箱配置 | 迁移活动运营链路 |
