...
3. **迭代节奏与当前轮次**
   ...
   - **当前状态**：`admin-web` 中已阶段性落地 **Phase 0～Phase 11**；**Phase 12 收尾部分完成** — **CI 门禁已完成 ✅**；§15.13 人工 QA **待人工/进行中**。Phase 8 活动项目详情子能力（奖品/投票绑定、获奖池）仍待补齐（§15.9）。
...
| P12  | 收尾优化       | 测试、类型补齐、构建优化、遗留样式与路径映射清理                                                                 | **部分完成**：**CI 门禁已完成 ✅**；§15.13 人工 QA **待人工/进行中**              |
...
| `schoolDoctor/` | 8 个子模块；独立路由 `router/schoolDoctor/index.js`；需求见 `schoolDoctor/校医需求.md` | P11 | 已迁移至 admin-web iews/school-doctor/ |
...
全量重构按“基座优先、低风险 CRUD 先行、复杂流程后置”的节奏推进。当前 **Phase 12 收尾部分完成** — **P12 CI 门禁已完成 ✅**（§15.13）；§15.13 别名审计、跨模块回归、校医八路由冒烟 **待人工/进行中**。Phase 8 活动项目详情子能力仍待补齐（§15.9）；**Phase 9 内容代码已落地 ✅**（§15.10），待人工 QA。
...
| Phase 9   | 内容管理             | 公告、动态内容、文章、讨论管理（9 子路由已接入）                                              | **代码完成 ✅**；人工 QA 待做 |
| Phase 10  | 宿舍管理             | 住宿生、空间管理、属性与分配规则、床位分配                                                    | 对齐 `dorm/`             |
| Phase 11  | 校医管理             | 学生档案、医疗信息、就诊、用药、传染病、体检等 8 子模块                                       | 对齐 `schoolDoctor/`     |
| Phase 12  | 收尾优化             | CI 门禁已通过；别名审计、跨模块回归、校医冒烟等待人工                                         | **部分完成**（CI ✅；人工 QA 进行中） |
...
**当前状态（2026-06-10）**：**代码已落地 ✅**；§15.10 人工 QA 待做（富文本/上传/详情预览、权限按钮冒烟）。

内容管理按旧菜单层级迁移：

1. 公告内容 → `/content/announcement` ✅
2. 动态内容：一周食谱、校园生活 → `/content/moment/food-weekly`、`/content/moment/school-life` ✅
3. 文章管理：文章内容、文章分类 → `/content/article/list`、`/content/article/category` ✅
4. 讨论管理：内容列表、讨论标签、讨论评论、点赞收藏 → `/content/discussion/{list,tag,comment,like-save}` ✅
...
**当前状态**：**部分完成** — **P12 CI 门禁已完成 ✅**；下列 **§15.13 人工项待人工/进行中**（非 CI 阻塞）。

**P12 CI 门禁（`admin-web`，2026-06-10 终验）**：

| 命令 | 结果 |
| --- | --- |
| `npm run type-check` | **PASS** ✅ |
| `npm run lint` | **PASS** ✅（0 errors，0 warnings；由 764 problems 清零） |
| `npm run lint:style` | **PASS** ✅（0 errors；`.stylelintrc.cjs` deep 伪类、program-cards media range、内容 detail `overflow-wrap` 等） |
| `npm run build` | **PASS** ✅ |

- [x] 全量 `type-check`、`lint`、`lint:style`、`build`（CI 门禁）

**§15.13 待人工 QA / 进行中**：

- [ ] 补齐 P9～P11 的 `MENU_PATH_ALIASES` 与隐藏路由（别名全量审计）。
