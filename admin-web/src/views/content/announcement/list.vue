<template>
  <section class="uni-list-page">
    <div class="uni-list-page__header">
      <div>
        <h1>{{ $t('content.announcement.pageTitle') }}</h1>
        <p>{{ $t('content.announcement.pageDesc') }}</p>
      </div>
      <div class="uni-list-page__header-actions">
        <el-button v-uni-permission="'announcement_add'" type="primary" @click="openForm('add')">
          {{ $t('content.add') }}
        </el-button>
      </div>
    </div>
...
import AnnouncementFormDialog from './components/form-dialog.vue'
import { activeOpts, searchForm, tableCols, urgencyLevelOpts } from './list.config'
import { contentAnnouncementApi, contentCommonApi } from '@/api'
import ListTableEmpty from '@/components/list-table-empty/index.vue'
import { useListTableEmpty } from '@/composables/use-list-table-empty'
import type { SchoolOptionRecord } from '@/types/modules/membership'
import type { ContentAnnouncementRecord as Row } from '@/types/modules/content-announcement'
...
const urgencyOptions = computed(() => urgencyLevelOpts(t))
const visibleOptions = computed(() => activeOpts(t))

const searchCfg = computed(() =>
  searchForm(t, schoolOptions.value, defaultSchoolId.value ?? undefined)
)
const columns = computed(() => tableCols(t, urgencyOptions.value, visibleOptions.value))
...
const loadData: UniTableRequest = async ({ pageNo: current, pageSize: size, filters: f }) => {
  const result = await contentAnnouncementApi.page.get({ current, size, ...f })
...
const removeRow = async (row: Row) => {
  try {
    await ElMessageBox.confirm(t('content.announcement.confirmDelete'), t('content.delete'), {
  ...
  await contentAnnouncementApi.delete.post({ id: row.id })
