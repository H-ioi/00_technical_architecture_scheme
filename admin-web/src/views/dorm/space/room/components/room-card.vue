  ...
  <el-card
    ...
    <div class="dorm-room-card__body">
      <div class="dorm-room-card__row dorm-room-card__row--split">
        <span>{{ $t('dorm.room.fieldCreatedAt') }}Ôºö{{ room.created_at || '‚Ä? }}</span>
        <span>{{ $t('dorm.room.fieldUpdatedAt') }}Ôºö{{ room.updated_at || '‚Ä? }}</span>
      </div>
      <div class="dorm-room-card__row">
        <span>{{ $t('dorm.room.fieldCampus') }}Ôº?/span>
        <span>{{ campusName }}</span>
      </div>
      <div class="dorm-room-card__row dorm-room-card__row--split">
        <span>
          {{ $t('dorm.room.fieldProject') }}Ôº?          <el-tooltip
            v-if="room.project?.name && room.project.name.length > 30"
            :content="room.project.name"
            placement="top">
            <span>{{ room.project.name.slice(0, 30) }}...</span>
          </el-tooltip>
          <span v-else>{{ room.project?.name || '‚Ä? }}</span>
        </span>
        <span>{{ $t('dorm.room.fieldBedCount') }}Ôºö{{ room.bed_usage_ratio || '‚Ä? }}</span>
      </div>
      <div class="dorm-room-card__row">
        <span>{{ $t('dorm.room.fieldStudents') }}Ôº?/span>
        <el-tooltip
        ...
        <span v-else>{{ room.student_names || '‚Ä? }}</span>
      </div>
    </div>
...
const campusName = computed(
  () =>
    props.floor.building?.school?.en_name || props.floor.building?.school?.cn_name || '‚Ä?
)

/** ‰∏éÊóßÁâà‰∏ÄËá¥Ôºögender 1/2 ‰∏éÂç°ÁâáÂ∫ïËâ?class Êò†Â∞Ñ‰∏éË°®ÂçïÊ†áÁ≠æÁõ∏Âè?*/
